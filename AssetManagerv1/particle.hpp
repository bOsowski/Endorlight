...
//  AssetManagerv1
//
//  Created by Denis Flynn on 11/11/2018.
...
public:
    ...
    const int getDissolutionRate(void) const {
        return m_dissolutionRate;
    }

    const int getNumberOfParticles(void) const {
        return m_particles.size();
    }

    const float getParticleSpeed(void) const {
        return m_particleSpeed;
    }

    const std::string getNumberOfParticlesString(void) const;

    void setCanvasSize(sf::Vector2u newSize) {
